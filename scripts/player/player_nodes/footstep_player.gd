...
@onready var particle_scene: CPUParticles2D = $FootstepsParticules
...
func play_footstep_particles(velocity_node_path: NodePath) -> void:
	...
	if velocity_node == null:
		push_error("play_footstep_particles(): Invalid node path: %s" % velocity_node_path)
		return
...
func play_footstep_sound() -> void:
	var material = _get_floor_material()
	match material:
		"stone":
		...
		"wood":
			AudioManager.create_2d_audio_at_location_with_culling(global_position, SoundEffect.SOUND_EFFECT_TYPE.ON_PLAYER_FOOTSTEP_WOOD)
		"metal":
			AudioManager.create_2d_audio_at_location_with_culling(global_position, SoundEffect.SOUND_EFFECT_TYPE.ON_PLAYER_FOOTSTEP_METAL)
		null, "":
			AudioManager.create_2d_audio_at_location_with_culling(global_position, SoundEffect.SOUND_EFFECT_TYPE.ON_PLAYER_FOOTSTEP_STONE)
...
func _get_floor_material() -> String:
	...
	if last_floor_collision == null: return ""
	...
	if collider is TileMapLayer:
		...
		if data: return data.get_custom_data("material")
	return ""
